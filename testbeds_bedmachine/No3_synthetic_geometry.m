%% This geometry type is STEEP UPWARD JUMP
% with an overdeepening and ridge
%% define parameters
% Geometry parameters
...
width = 7200; % fjord valley width in meter. must be even numbers and multiples of 2n
length = 35000; 
fjordwater_length = 3000; % where ice is absent (did i use this?)
bed_shearstress = 300000; % kPa
...
wave1_x = -wl1:n:0; % also keep the latter 
od1 = ap1*sin(pi/wl1.*wave1_x);
wave2_x = -wl2:n:0;
od2 = -1*ap2*sin(pi/wl2.*wave2_x);
wave0_x = -wl0*0.75:n:0; % keep the latter 75% 
od0 = od1(1)*ones(size(wave0_x));

% low amplitude wave filling the intervals
wave_space1 = 5000;
ap_av = (ap0+ap1+ap2)/3; % aplitude: mean of the previous ones
% amplitude co-efficient
ap_sp = ap_coef*ap_av;
sp_wave_x = 0:n:wave_space1;
sp_wave   = ap_sp*sin(pi/wave_space1.*sp_wave_x);
sp_wave_0constant = 0.*sp_wave_x;

% piecing together
% concatenate
thalweg = [od0, od1, od2, sp_wave];

N_x_temp = numel(wave0_x) + numel(wave1_x) + numel(wave2_x) + numel(sp_wave_x);
...
x = 0:n:(N_x_tot-1)*n;
% concatenate
thalweg = [od0, od1, od2, sp_wave, flat_filler];
...
thalweg(1:I) = M;
if all(isnan(thalweg))
    disp('There is NaN in thalweg, try changing the sampling interval')
    return
end

thalweg_base = thalweg;

% add the error function to the bed shape, for "steep upward jump" type
thalweg_steepup_x_last = 25000; % from the plot, the last x value of a complete error fx
flat_part = (thalweg_steepup_x_last - (bottom_top_long/1)*2)/2;
if flat_part < 0
    disp(' Flat_part is negative; increase thalweg_steepup_x_last')
    return
end

thalweg_steepup_x = 1:n:thalweg_steepup_x_last;
half_bottom_top_long = 0.5*bottom_top_long;
half_bottom_top_diff = 0.5*bottom_top_diff;
thalweg_steepup = half_bottom_top_diff.*erf((1/half_bottom_top_long).*(thalweg_steepup_x-half_bottom_top_long-flat_part));
% add the plateu part in the end
thalweg_steepup_x_rest = (thalweg_steepup_x_last + n):n:x(end);
thalweg_steepup_rest   = thalweg_steepup(end)*ones(size(thalweg_steepup_x_rest));
% combine
thalweg_steepup   = [thalweg_steepup, thalweg_steepup_rest];
thalweg_steepup_x = [thalweg_steepup_x, thalweg_steepup_x_rest];
% interpolate back to the original x
thalweg_steepup = interp1(thalweg_steepup_x, thalweg_steepup, x,'linear','extrap');

% combine steepup and the overdeepenings
thalweg = thalweg_base + thalweg_steepup;
...
syn_3.s = syn_s;
syn_3.h = syn_h;
syn_3.ocean_mask = ocean_mask;
syn_3.ice_mask   = ice_mask;
syn_3.end_year   = melt_years{end};
syn_3.fric_coef  = fric_coef;
syn_3.transient_fric_coef = transient_fric_coef;
syn_3.shelf_melt.transient_melt = transient_melt;
syn_3.shelf_melt.constant_melt  = constant_melt;
syn_3.shelf_melt.melt_years     = melt_years;
syn_3.SMB_grad = SMB_grad;
syn_3.SMB_cons = SMB_cons;
syn_3.rheoB = rheoB;
syn_3.transient_rheoB = transient_rheoB;
syn_3.description = 'Steep upward knickpoint geometry';
save('Synthetic glaciers/syn_3','syn_3')

%% Save the perturbed forcings for sensitivity runs
%sens_3.shelf_melt = sens_shelf_melt;
sens_3.fric_coef  = sens_fric_coef;
sens_3.rheoB      = sens_rheoB;
% count the exisiting number of fields (must be before N_test)
N_vars = numel(fieldnames(sens_3));
sens_3.N_vars     = N_vars;

sens_3.N_test     = N_sens;
save('Sensitivity/sens_3','sens_3')