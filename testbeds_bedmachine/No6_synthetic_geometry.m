%% This geometry inherits the double-overdeepening geometry. 
% It has variable fjord width (constriction)
...
width_amp = 0.3*width; % variable fjord width, wave amplitude
width_wl  = width;     % variable fjord width, wave length
length = 45000; % total length of the fjord (along flow direction)
bed_shearstress = 100000; % 100kPa 
g = 9.81; % gravitational constant

if tot_wid < width + width_amp
    disp('Total width should be no smaller than max fjord width')
    return;
end
...
width_baseline   = width*ones(numel(thalweg),1);
width_var_x = linspace(-width_wl, 0, numel(thalweg));
width_var   = width_amp*sin(pi/width_wl.*width_var_x);
% combine
width_x  = width_baseline + transpose(width_var);
...
% first need to find the variable fjord boundary elements
% which separate positive and negative elevations
syn_b(syn_b>0) = 0;
left_bound_i = zeros(N_tot_wid, 1);
right_bound_i  = zeros(N_tot_wid, 1);
% we still need to find the index of the boundary elements
for i = 1:N_x_tot
    this_b = syn_b(:, i);
    bounds_i = find(this_b<0);
    left_bounds_i  = bounds_i(bounds_i < thalweg_i); 
    right_bounds_i = bounds_i(bounds_i > thalweg_i);
    left_bound_i(i)  = left_bounds_i(1); 
    right_bound_i(i) = right_bounds_i(end);
end
% find the numerical derivative at the edge of the overdeepening
grad_od_right = zeros(numel(thalweg),1);
grad_od_left  = zeros(numel(thalweg),1);
for i = 1:numel(thalweg)
    grad_od_right(i) = (syn_b(right_bound_i(i),i)-syn_b(right_bound_i(i)-2,i))/(2*n);
    grad_od_left(i)  = (syn_b(left_bound_i(i),i)-syn_b(left_bound_i(i)+2,i))/(2*n);
end
...
syn_side_wall = zeros(size(syn_b));
for i = 1:numel(thalweg)
    beta_left  = sqrt(pi)/(2*alpha)*grad_od_left(i);
    beta_right = sqrt(pi)/(2*alpha)*grad_od_right(i);
    N_side_wall_right = size(syn_b,1) - right_bound_i(i);
    N_side_wall_left  = left_bound_i(i) - 1;
    x_side_wall_right = transpose(1*n:n:N_side_wall_right*n);
    x_side_wall_left  = x_side_wall_right;
    % right wall
    syn_side_wall(end-N_side_wall_right+1:end,i) = alpha.*erf(beta_right.*x_side_wall_right);
    % left wall: flip
    syn_side_wall(N_side_wall_left:-1:1,i)  = alpha.*erf(beta_left.*x_side_wall_left);
end
% combine the side wall and the fjord valley elevation
syn_b = syn_b + syn_side_wall;
...
% % Time dependent melting rate; time unit in years
 melt_years = {5, 10, 200}; % 0-1st, stationary; 1st-2nd, warming; 2nd-3rd, cooling
...
basal_melt  = 30*ones(size(syn_b));
basal_melt1 = basal_melt;
basal_melt2 = 30*ones(size(syn_b));
basal_melt3 = 30*ones(size(syn_b));
...
figure
subplot(2,1,1)
mesh(X,Y,syn_b);hold on;mesh(X,Y,syn_s);hold on;mesh(X,Y,flot_pf)
subplot(2,1,2)
mesh(data.X, data.Y, data.b); hold on; % bed
mesh(data.X, data.Y, data.s); hold off
...
syn_6.s = syn_s;
syn_6.h = syn_h;
syn_6.ocean_mask = ocean_mask;
syn_6.ice_mask   = ice_mask;
syn_6.end_year   = melt_years{end};
syn_6.fric_coef = fric_coef;
syn_6.shelf_melt.transient_melt = transient_melt;
syn_6.shelf_melt.constant_melt  = constant_melt;
syn_6.shelf_melt.melt_years     = melt_years;
syn_6.SMB = SMB;
syn_6.description = 'Calving on the Ridge type (CR) as described in Wood et al., 2021';
save('Synthetic glaciers/syn_6','syn_6')
