%% This geometry type is plain parabolic glacier valley with no basal variation
%  Model index = 00. Different from No0, it is narrower and thicker, meant to represent
%  glaciers with higher wall drag in force balance.
%% define parameters
% Geometry parameters
max_len = 400; % cutoff distance of flowline, in km
n = 150; % sine wave sampling points
ap_coef = 0.05; % filler wave amplitude
tot_wid = 3600*2; %  total width of this synthetic fjord in meter
elev_bench = 450; % zero benchmark line for parabola
fjord_wid = 2700; % fjord valley width in meter. must be even numbers and multiples of 2n
tot_length = 35000; % fjord/glacier total length in meter
...
s_margin_width = 4; % length for each shear margin side, 3*150m
s_margin_weakcoef = 0.3; % weaken to _% of the default value
fric_coef_ampfactor = 0.3; % weaken to _% of the default fric coef
slippatch_loc_x = 60; % the x index coordinate of the gaussian slippery patch
slippatch_wid = 0.5*(fjord_wid/2); % sigma in the gaussian function; half of fjord half width
...
width_x = fjord_wid*ones(numel(thalweg),1);
...
wid_left_i  = thalweg_i - (fjord_wid/n/2);
wid_right_i = thalweg_i + (fjord_wid/n/2);
...
syn_00.Y = Y;
syn_00.bed = syn_b;
syn_00.base = syn_base;
syn_00.s = syn_s;
syn_00.h = syn_h;
syn_00.ocean_mask = ocean_mask;
syn_00.ice_mask   = ice_mask;
syn_00.end_year   = melt_years{end};
syn_00.fric_coef  = fric_coef;
syn_00.transient_fric_coef = transient_fric_coef;
syn_00.shelf_melt.transient_melt = transient_melt;
syn_00.shelf_melt.constant_melt  = constant_melt;
syn_00.shelf_melt.melt_years     = melt_years;
syn_00.SMB_grad = SMB_grad;
syn_00.SMB_cons = SMB_cons;
syn_00.rheoB = rheoB;
syn_00.transient_rheoB = transient_rheoB;
syn_00.description = 'Parabolic trunk. No basal perturbation';
save('Synthetic glaciers/syn_00','syn_00')

%% Save the perturbed forcings for sensitivity runs
%sens_3.shelf_melt = sens_shelf_melt;
sens_00.fric_coef  = sens_fric_coef;
sens_00.rheoB      = sens_rheoB;
% count the exisiting number of fields (must be before N_test)
N_vars = numel(fieldnames(sens_00));
sens_00.N_vars     = N_vars;

sens_00.N_test     = N_sens;
save('Sensitivity/sens_00','sens_00')