...
n = 150; % sine wave sampling points; same as BedMachine interval
ap_coef = 0.05; % filler wave amplitude
tot_wid = 4500; % 4.5 km total width of this synthetic fjord
elev_bench = 300; % zero benchmark line for parabola
width = 1200; % overdeepening width in meter. must be even numbers
length = 90000; % total length of the fjord (along flow direction)
...
openfig('Plots/Dennis flowline/mesh_fig_bh/glacier0003_flowline03_mesh_bh.fig');
...
wave1_x = -wl1*0.80:n:0; % also keep the latter 
...
x = 0:n:(N_x_tot-1)*n;
% concatenate
thalweg = [od0, od1, sp_wave, od2, flat_filler];
...
hp = [0, 40]; % at the ice shelf front, the height is 30 m
if hp(1) > ep(1)
    disp('the x of the end point should be larger than the tongue head')
    return
end
if hp(2) > ep(2)
...
melt_years = {5, 17, 29}; % 0-1st, stationary; 1st-2nd, warming; 2nd-3rd, cooling
...
syn_2.s = syn_s;
syn_2.h = syn_h;
syn_2.ocean_mask = ocean_mask;
syn_2.ice_mask   = ice_mask;
syn_2.end_year   = melt_years{end};
syn_2.fric_coef  = fric_coef;
syn_2.shelf_melt.transient_melt = transient_melt;
syn_2.shelf_melt.constant_melt  = constant_melt;
syn_2.shelf_melt.melt_years     = melt_years;
syn_2.SMB = SMB;
syn_2.description = 'Deep fjord with Warm water (DW) as described in Wood et al., 2021';
save('Synthetic glaciers/syn_2','syn_2')
