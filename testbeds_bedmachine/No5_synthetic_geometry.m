%% This geometry inherits the double-overdeepening geometry, but with a falling/decreasing
% basal elevation trend, realized by rotation
...
degree = -20; % basal elevation rotation degree
fjordwater_length = 3000; % length of the fjord water section (ice is absent)
...
%% Adding fjord water (ice is absent)
% basically just replicating the crossection of fjord geometry at (:,1)
N_crosssection = fjordwater_length/n;
% change syn_b
syn_b_water = repmat(syn_b(:,1), 1, N_crosssection);
syn_b = [syn_b_water, syn_b];

% Change the X and Y meshgrid
x_water = -n:-n:N_crosssection*(-n);
X_water = repmat(x_water, size(Y,1), 1);
y_water = Y(:,1);
Y_water = repmat(y_water, 1, numel(x_water));
% concatenate
X = [X_water, X];
Y = [Y_water, Y];
% displace the values so that the left end of x starts at 0
X = X - fjordwater_length;

%% Rotate the basal topography
% call the rotation_wrt_point function, but the Y meshgrid is actually
% elevation (z)
for i = 1:N_tot_wid
    this_b_row = syn_b(i,:);
    this_x_row = X(1,:);
    point  = [this_x_row(1), this_b_row(1)];
    angle  = (degree/180)*pi;
    % rotate wrt the left-most point (prob. on the shelf)
    [~, b_rotated] = rotation_wrt_point(this_x_row, this_b_row, point, angle);
    syn_b(i,:) = transpose(b_rotated);
end
...
syn_5.s = syn_s;
syn_5.h = syn_h;
syn_5.ocean_mask = ocean_mask;
syn_5.ice_mask   = ice_mask;
syn_5.end_year   = melt_years{end};
syn_5.fric_coef = fric_coef;
syn_5.shelf_melt.transient_melt = transient_melt;
syn_5.shelf_melt.constant_melt  = constant_melt;
syn_5.shelf_melt.melt_years     = melt_years;
syn_5.SMB = SMB;
syn_5.description = 'Calving on the Ridge type (CR) as described in Wood et al., 2021, with a falling basal elevation upstream';
save('Synthetic glaciers/syn_5','syn_5')
